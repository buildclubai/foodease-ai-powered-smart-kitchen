...
import time
import io
from recipe_tracker import add_save_recipe_button, save_recipe
...
def create_recipe_card(recipe, recipe_details):
    ...
    # Add save button
    add_save_recipe_button(recipe, key_info)
...
def main():
    ...
    if st.session_state.analysis_complete:
        ...
        if 'current_recipes' not in st.session_state or st.button("🍳 Generate Recipes", key="generate_recipes"):
            st.subheader("🍽️ Generated Recipes")
            with st.spinner("🔍 Searching for delicious recipes..."):
                st.session_state.current_recipes = get_recipes_from_spoonacular(list(st.session_state.items_info.keys()))
                # Clear previous recipe details
                st.session_state.recipe_details = {}
        
        # Display recipes if they exist in session state
        if st.session_state.current_recipes:
            for i in range(0, len(st.session_state.current_recipes), 2):
                cols = st.columns(2)
                for j in range(2):
                    if i + j < len(st.session_state.current_recipes):
                        recipe = st.session_state.current_recipes[i + j]
                        with cols[j]:
                            # Generate or retrieve recipe details
                            recipe_id = recipe['id']
                            if recipe_id not in st.session_state.recipe_details:
                                with st.spinner(f"✨ Generating details for {recipe['title']}..."):
                                    st.session_state.recipe_details[recipe_id] = generate_recipe_details_groq(recipe)
